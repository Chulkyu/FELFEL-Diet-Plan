...
export default function App() {
  ...
  if (!apiKey) {
    return <ApiKeySetup onApiKeySubmit={handleApiKeySave} />;
  }

  return <DietPlannerApp apiKey={apiKey} onResetApiKey={handleResetApiKey} />;
}
    ...
    return (
        <div className="min-h-screen flex items-center justify-center p-4">
            <div className="w-full max-w-md bg-slate-800 p-8 rounded-2xl shadow-2xl border border-slate-700">
                <h2 className="text-2xl font-bold text-center text-slate-200 mb-2">Enter Your API Key</h2>
                ...
                <form onSubmit={handleSubmit}>
                    <input
                        ...
                        className="w-full bg-slate-700 border border-slate-600 rounded-md py-3 px-4 text-white focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 mb-4"
                    />
                    <button type="submit" className="w-full bg-cyan-500 hover:bg-cyan-600 text-white font-bold py-3 px-4 rounded-lg transition-colors">
                        Save and Start
                    </button>
                </form>
                 <p className="text-center text-slate-500 mt-4 text-xs">
                    You can get your key from <a href="https://aistudio.google.com/" target="_blank" rel="noopener noreferrer" className="text-cyan-400 hover:underline">Google AI Studio</a>.
                </p>
            </div>
        </div>
    );
...
const DietPlannerApp: React.FC<{apiKey: string; onResetApiKey: () => void;}> = ({ apiKey, onResetApiKey }) => {
  ...
  const handleScan = useCallback(async (imageDataUrl: string, mimeType: string) => {
    ...
    try {
      ...
      const result = await analyzeFoodLabel(apiKey, base64Data, mimeType);
  ...
  const handleGetRecommendations = useCallback(async () => {
      ...
      try {
          const result = await recommendMeals(apiKey, remainingNeeds);
        ...
        <header className="flex justify-between items-center mb-8">
          <h1 className="text-3xl sm:text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-teal-500">
           ...
           <div className="flex items-center gap-4">
             ...
             <button onClick={onResetApiKey} title="Reset API Key" className="bg-red-800 hover:bg-red-700 text-white font-bold p-2 rounded-lg transition-colors">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                {/* Meal Recommendations */}
                 { (isLoading && recommendations.length === 0) &&
                  <section className="bg-slate-800/50 backdrop-blur-sm p-6 rounded-2xl shadow-2xl border border-slate-700" ref={animationParent}>
                      ...
                      <Spinner message="Generating recommendations..."/>
                  </section>
                 }
              ...
              <div className="space-y-6" ref={animationParent}>
                <section className="bg-slate-800/50 backdrop-blur-sm p-6 rounded-2xl shadow-2xl border border-slate-700">
                 ...
                 {isLoading && !scannedItem && !recommendations.length && <Spinner message="Analyzing label..." />}
